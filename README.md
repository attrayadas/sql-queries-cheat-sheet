...
5. UUID
6. Arrays: Stores an array of strings, numbers, etc
7. JSON
...
`CREATE TABLE account_job(user_id INTEGER REFERENCES acount(user_id), job_id INTEGER REFERENCES job(job_id), hire_date TIMESTAMP);`

### INSERT:
|                                              | General Syntax                                                                                                |
|----------------------------------------------|---------------------------------------------------------------------------------------------------------------|
|                                              | INSERT INTO table (column1, column2, ...)  VALUES(value1, value2, ...), (value1, value2, ...), ...;           |
| INSERT allows you to add in rows to a table  | INSERT INTO table (column1, column2,...)  SELECT column1, column2, ...  FROM another_table   WHERE condition; |

eg:

`INSERT INTO acount(username, password, email, created_on) VALUES ('Attraya', 'password', 'attraya@gmail.com', CURRENT_TIMESTAMP);`
...
| We can delete rows based on their presence in other tables | DELETE FROM tableA  USING tableB   WHERE tableA.id=TableB.id; |
...
| Adding Columns     | ALTER TABLE table_name ADD COLUMN new_col TYPE;                               |
| Removing Columns   | ALTER TABLE table_name DROP COLUMN col_name;                                  |
...
| Alter constraints  | ALTER TABLE table_name ALTER COLUMN col_name ADD CONSTRAINT constraint_name;  |
...
| Remove all dependencies: ALTER TABLE table_name DROP COLUMN col_name CASCADE;            | 
| Check for existence to avoid error: DROP COLUMN IF EXISTS col_name;                      | 
| Drop multiple columns: ALTER TABLE table_name DROP COLUMN col_one, DROP COLUMN col_two;  | 
...
`CREATE TABLE employees(<br/>emp_id SERIAL PRIMARY KEY, first_name VARCHAR(50) NOT NULL, last_name VARCHAR(50) NOT NULL, birthdate DATE CHECK (birthdate>'1900-01-01'), hire_date DATE CHECK (hire_date > birthdate), salary INTEGER CHECK (salary > 0));`
